...
function LaptopDetail({ Laptops }) {
  console.log('latopdtail');
  let { id } = useParams();
  
  let laptopsInfo = Laptops.find((data, i) => {
    return data.id == id;
  });
  ...
  return (
    <div>
      <Card style={{ width: "50%", margin: "0 auto" }}>
        <Card.Img variant="top" src={laptopsInfo.imgPath} />
        <Card.Body>
          <Card.Title>{laptopsInfo.itemName}</Card.Title>
          <Card.Text></Card.Text>
        </Card.Body>
        <ListGroup className="list-group-flush">
          <ListGroup.Item>{laptopsInfo.content}</ListGroup.Item>
          <ListGroup.Item>{laptopsInfo.price}</ListGroup.Item>
        </ListGroup>
        <Card.Body>
