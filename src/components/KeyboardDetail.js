...
function KeyboardDetail({ keyboards }) {
  let { id } = useParams();
  
  let keyboardsInfo = keyboards.find((data, i) => {
    return data.id == id;
  });
  ...
  return (
    <div>
      <Card style={{ width: "50%", margin: "0 auto" }}>
        <Card.Img variant="top" src={keyboardsInfo.imgPath} />
        <Card.Body>
          <Card.Title>{keyboardsInfo.itemName}</Card.Title>
          <Card.Text></Card.Text>
        </Card.Body>
        <ListGroup className="list-group-flush">
          <ListGroup.Item>{keyboardsInfo.content}</ListGroup.Item>
          <ListGroup.Item>{keyboardsInfo.price}</ListGroup.Item>
        </ListGroup>
        <Card.Body>
